...
(function(){
  ...
  const STATE = {
    legajo:"",
    codes:[],      // únicos del paso 1
    rows:[],       // filas por sector/cod (paso 2)
    fills:{},      // paso 2
    resumen:{}     // paso 3: por código
  };
  ...
  function payloadPaso2(){
    ...
    return { kind:"paso2", ts: new Date().toISOString(), legajo: STATE.legajo, rows };
  }
  ...
  function payloadPaso3(){
    const rows = STATE.codes.map(code => ({
      codArt: code,
    ...
    return { kind:"paso3", ts: new Date().toISOString(), legajo: STATE.legajo, rows };
  }
  ...
  $("limpiarBtn").addEventListener("click", resetAllToStart);
  ...
  $("enviarSheetBtn").addEventListener("click", async () => {
    const msg = $("msgSend");
    msg.textContent = "";
    msg.className = "msg";
    ...
    try{
      showOverlay(true, "Enviando…", "Guardando Paso 2");
      await postJSON(payloadPaso2());
  ...
  $("reset3Btn").addEventListener("click", resetPaso3);

  $("enviar3Btn").addEventListener("click", async () => {
    const msg = $("msg3");
    msg.textContent = "";
    msg.className = "msg";

    try{
      showOverlay(true, "Enviando…", "Guardando Paso 3");
      await postJSON(payloadPaso3());
      await new Promise(r => setTimeout(r, 500));
      showOverlay(false);

      // Termina: vuelve al inicio
      resetAllToStart();
