 ...
 * NOTA: Este archivo NO incluye el MAPEO. Tu mapeo debe definir:
 *   window.PLANIMETRIA_MAP = { "066": [{orden: 61, sector:"A61", codArt:"066"}, ...], ... }
 ***********************/
(function () {
  ...
  function renderTab2() {
    ...
    for (const r of STATE.rows) {
      ...
      inpS.maxLength = 2; // Paso 2 sigue en 2 dígitos como dijiste
      inpS.placeholder = "0";
  ...
  function renderTab3() {
    ...
    for (const code of STATE.codes) {
      ...
      function clamp3(codeKey, field, el) {
        el.value = onlyDigits(el.value).slice(0, 3);
        STATE.resumen[codeKey][field] = el.value;
      }

      inpPA.addEventListener("input", () => clamp3(code, "pickings", inpPA));
      inpPFC.addEventListener("input", () => clamp3(code, "pfc", inpPFC));
      inpMT.addEventListener("input", () => clamp3(code, "transito", inpMT));
